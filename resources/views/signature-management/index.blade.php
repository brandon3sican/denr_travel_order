            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3">
                                    <div class="flex items-start">
                                        ...
                                        @endphp
                                        <div class="h-10 w-10 rounded-full {{ $bgColor }} flex-shrink-0 flex items-center justify-center text-white font-bold">
                                            {{ $firstLetter }}
                                        </div>
                                        <div class="ml-3">
                                            <div class="font-medium text-gray-900">
                                                {{ $row->first_name }}{{ $row->middle_name ? ' ' . $row->middle_name : '' }} {{ $row->last_name }}{{ $row->suffix ? ' ' . $row->suffix : '' }}
                                            </div>
                                            <div class="text-gray-500 text-xs mb-1">{{ $row->user_email }}</div>
                                            <div class="space-y-0.5">
                                                <div class="text-gray-700"><span class="font-medium">Position:</span> {{ $row->position_name ?? 'N/A' }}</div>
                                                <div class="text-gray-700"><span class="font-medium">Assignment:</span> {{ $row->assignment_name ?? 'N/A' }}</div>
                                                @if($row->signature_id)
                                                    <div class="text-green-700 text-xs">Has signature • Updated {{ optional($row->signature_updated_at)->format('M d, Y h:i A') }}</div>
                                                @else
                                ...
                                <td class="px-4 py-3">
                                    <button type="button"
                                            ...
                                            data-employee-email="{{ $row->user_email }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="2" class="px-6 py-4 text-center text-sm text-gray-500">No employees found.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
        ...
        <footer class="bg-white border-t border-gray-200 mt-4">
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                <div class="text-center">
                    <p class="text-sm text-gray-500">&copy; {{ date('Y') }} Department of Environment and Natural Resources. All rights reserved.</p>
                </div>
            </div>
        </footer>
    </div>
            ...
            <form id="resetSigForm" method="POST">
                ...
                <div class="p-6 space-y-3">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="email" name="confirm_email" id="confirmEmailInput" class="w-full border rounded px-3 py-2 text-sm" required placeholder="employee@example.com">
                    </div>
                    <label class="flex items-start space-x-2 text-sm text-gray-700">
                        <input type="checkbox" name="ack_reset" id="ackReset" class="h-4 w-4 text-red-600 border-gray-300 rounded" required>
                        <span>I understand this will remove the existing signature and the user will need to provide a new one.</span>
                    </label>
                    <div class="flex justify-end space-x-2 pt-2">
                        <button type="button" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 rounded" onclick="closeResetModal()">Cancel</button>
                        <button type="submit" id="confirmResetBtn" class="px-4 py-2 text-sm font-semibold text-white bg-red-600 hover:bg-red-700 rounded">Confirm Reset</button>
                    </div>
                </div>
            </form>
        </div>
