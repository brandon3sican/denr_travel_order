                                        ...
                                        @forelse($travelOrders as $order)
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-3 py-2 whitespace-nowrap text-center">
                                                    <button data-order-id="{{ $order->id }}" data-action="approve"
                                                        class="approve-btn text-green-600 hover:text-green-900 border border-green-600 px-2 py-1 rounded mr-3 w-25 {{ $order->status->name !== 'For Approval' ? 'opacity-50 cursor-not-allowed' : '' }}"
                                                        {{ $order->status->name !== 'For Approval' ? 'disabled' : '' }}>
                                                        <i class="fas fa-check"></i> Approve
                                                    </button>
                                                    <button data-order-id="{{ $order->id }}" data-action="reject"
                                                        class="reject-btn text-red-600 hover:text-red-900 border border-red-600 px-2 py-1 rounded mr-3 w-20 {{ $order->status->name !== 'For Approval' ? 'opacity-50 cursor-not-allowed' : '' }}"
                                                        {{ $order->status->name !== 'For Approval' ? 'disabled' : '' }}>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.querySelectorAll('.approve-btn').forEach(button => {
                    button.addEventListener('click', async function() {
                        ...
                        try {
                            const confirmed = await approve(orderId);
                            if (confirmed) {
                                // Password was verified, now show the approval modal
