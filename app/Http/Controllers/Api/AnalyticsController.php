...
class AnalyticsController extends Controller
{
    ...
    private function getLineChartData($query, $period, $month = null, $year = null)
    {
        ...
        $statuses = [
            'pending' => [
                'query' => function($q) { $q->whereIn('status_id', [1, 2]); }, // Combined For Recommendation (1) and For Approval (2)
                'breakdown' => [
                    'for_recommendation' => function($q) { $q->where('status_id', 1); },
                    'for_approval' => function($q) { $q->where('status_id', 2); }
                ]
            ],
            'completed' => [
                'query' => function($q) { $q->where('status_id', 4); } // Completed
            ],
            'cancelled' => [
                'query' => function($q) { $q->where('status_id', 5); } // Cancelled
            ],
            'disapproved' => [
                'query' => function($q) { $q->where('status_id', 6); } // Disapproved
            ]
        ];
